...
- Use `.claude/skills/agentic-workflow-designer/SKILL.md` to discover and confirm requirements.
...
If no preference, suggest a default:
- "I'd suggest Claude since you haven't mentioned a preference. Sound good?"

Map to `engine:` only when not the gh-aw default.
...
- [ ] Unnecessary defaults are omitted (only set `engine:` when overriding the gh-aw default)
