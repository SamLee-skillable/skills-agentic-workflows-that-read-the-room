...
**Load when**: The user asks how to run, compile, debug, or manage workflows from the command line; needs the MCP tool equivalent of a `gh aw` command; or is in a restricted environment without direct CLI access.
...
- "I'm in a restricted environment — how do I compile a workflow?"
...
- Use the MCP server `github-agentic-workflows` (configured in `.mcp.json`) when the `gh aw` CLI is not directly available in the current environment
